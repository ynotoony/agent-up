...
#        docs/progress.md——冻结历史档案（指针注记后零写入，development-process §11）：
#        任何 diff 即违规。文件不存在跳过（懒创建语义）；无 Git 基线（无 HEAD）输出
...
守卫对象:
  ...
  docs/progress.md                            冻结历史档案：任何 diff 即 FAIL。
...
check_frozen_file 'docs/progress.md'
