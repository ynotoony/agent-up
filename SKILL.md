...
5. **安装/补缺**：按 `references/templates/README.md` 的 manifest 生成 seed 五件套、命中的条件产物与三阶段角色合同（生成到 `docs/agent/roles/`）；生成件自带契约头；模板中"按项目填写"处用盘点与访谈结果填充，没问到写 `【待定：...】`。旧项目和部分治理只补缺失件，登记既有权威来源。道脚本落位：用户确认启用分级交付道快道时，从本包 `scripts/` 复制 `check-gates.sh`、`lane-commit.sh` 与 `generate-progress.sh`（现役状态投影生成器；未启用快道的既有项目迁移按 `references/old-project.md` §3 补缺行落位）到目标项目 `scripts/`（目录缺失则连同 `scripts/README.md` 按 `scripts-README.md.tmpl` 一并生成），逐件登记 generation manifest 与 `docs/agent/artifacts.yaml`（kind: script、lifecycle: Conditional、generated_from: agent-up/scripts/<文件名>）；未启用不复制，道脚本不可用的停止条件按生成的 development-process R-DP-031 照旧兜底。票务运维脚本落位（独立口径，不门控于快道启用——票务运维是记录层标配，非快道专属，与道脚本落位为两套独立门控）：目标项目采用任务票体系（`docs/issues/` 目录创建）时，随初始化/补缺从本包 `scripts/` 复制 `ticket-ops.sh` 到目标项目 `scripts/`（目录缺失则连同 `scripts/README.md` 一并生成，同上），逐件登记 generation manifest 与 `docs/agent/artifacts.yaml`（kind: script、lifecycle: Conditional、generated_from: agent-up/scripts/ticket-ops.sh）；收尾投影再生依赖同目录 `generate-progress.sh`（PATH 回退），目标项目未落位生成器时一并复制落位；未采用任务票体系不复制；包内 `agent-up/scripts/ticket-ops.sh` 为复制基线、不在包内运行（缺省 repo-root 取脚本所在目录的上一级，包内位置误运行会错根）。每个生成件登记 generation manifest（来源、日期、目标、用户确认、版本与恢复说明）；重复执行走 reconcile：检测到人工漂移即停并报告而非覆盖，写入前先备份保证可逆。
...
| `references/templates/README.md` | 生成治理骨架前：模板 manifest、seed 五件套映射、条件产物触发矩阵与角色合同模板 |
