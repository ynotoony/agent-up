...
5. **安装/补缺**：按 `references/templates/README.md` 的 manifest 生成 seed 五件套、命中的条件产物与三阶段角色合同（生成到 `docs/agent/roles/`）；生成件自带契约头；模板中"按项目填写"处用盘点与访谈结果填充，没问到写 `【待定：...】`。旧项目和部分治理只补缺失件，登记既有权威来源。道脚本与票务运维脚本落位（两套独立门控，启用判定均为用户决策，本流程不代做访谈）：道脚本在用户确认启用分级交付道快道时落位；票务运维脚本在目标项目采用任务票体系（`docs/issues/` 目录创建）时落位（记录层标配，非快道专属）。已确认的复制动作经本包 `scripts/install.sh` 执行（`install.sh --target <目标项目根> [--fast-lane] [--ticket-ops]`），复制集合、每件复制基线路径与登记 lifecycle 值的权威在数据文件 `scripts/install-policy.rules`（外置数据、引擎零专名，本文件不复写映射明细）；落位到目标项目 `scripts/`（目录缺失则由脚本创建并输出提示行，`scripts/README.md` 生成归执行体），逐件登记 generation manifest 与 `docs/agent/artifacts.yaml`（kind: script；lifecycle、generated_from 等十三字段建议值以脚本输出的登记建议块为准）；未启用对应门控不复制，未启用快道的既有项目迁移按 `references/old-project.md` §3 补缺行落位，道脚本不可用的停止条件按生成的 development-process R-DP-031 照旧兜底；包内 `agent-up/scripts/` 为复制基线、不在包内运行（install.sh 自带包内误运行守卫）。每个生成件登记 generation manifest（来源、日期、目标、用户确认、版本与恢复说明）；重复执行走 reconcile：检测到人工漂移即停并报告而非覆盖，写入前先备份保证可逆。
