...
| 道脚本（落位集合与每件基线见公开包 `scripts/install-policy.rules`） | 用户确认启用分级交付道快道时才补缺：经 agent-up 公开包 `scripts/install.sh`（`--target <目标项目根> --fast-lane`）复制落位目标项目 `scripts/`（集合构成、每件复制基线路径与登记 lifecycle 值以 `install-policy.rules` 数据为准，本表不复写；`scripts/` 目录缺失由脚本创建并输出提示行，`scripts/README.md` 生成归执行体），逐件登记 generation manifest 与 `docs/agent/artifacts.yaml`；未启用不补，道脚本不可用停止条件按 development-process R-DP-031 兜底；现役状态投影 `docs/progress-current.md` 由落位的投影生成器自 `docs/issues/index.json` 生成，生成器独占写。 |
| 票务运维脚本（落位集合同上数据源） | 目标项目采用任务票体系（`docs/issues/` 目录创建）时补缺（记录层标配，不门控于快道启用，与道脚本两套独立门控）：经 agent-up 公开包 `scripts/install.sh`（`--target <目标项目根> --ticket-ops`）复制落位目标项目 `scripts/`（含生成器回退依赖注记，集合以 `install-policy.rules` 为准），逐件登记 generation manifest 与 `docs/agent/artifacts.yaml`；未采用任务票体系不补。 |
