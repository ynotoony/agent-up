...
lifecycle: Live
read_when: 规划、拆票或写任务票前评估 Complexity 与 Requirement Profile 时；判定 C2/C3 触发矩阵命中、拆票与上下文策略或 Profile 七维证据时
trigger: C0-C3 触发条件、高风险面清单、拆票与上下文策略、Profile 七维含义或状态填法语义变化
...
<!-- Output: Agent Up 复杂度分级与 Requirement Profile 的产品事实源：C0-C3 权威表（等级/触发条件/默认拆票与上下文策略/最低交付证据/高风险面清单）与 D/B/I/U/S/M/O 七维 Profile 权威表（含义/Required 最低证据/状态填法）及两者正交原则。 -->
